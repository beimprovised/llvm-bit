...
#									if ! [ "$DOXYGEN_EXECUTABLE" = "" ] ; then
										...
										if [ "$PWD" = "$WORKING_PATH/$BUILD_PATH_PREFIX-$RELATIVE_SOURCE_PATH" ]; then
											...
											-DHAVE_CXX_ATOMICS_WITHOUT_LIB:INTERNAL=1 \
											-DHAVE_CXX_ATOMICS64_WITHOUT_LIB:INTERNAL=1 \
...
`#											-DCMAKE_ASM_COMPILER:FILEPATH="$INSTALL_ROOT/bin/$CC"` \
											...
											-DCUDALIB_INCLUDE_DIR:PATH=$ACCELERATOR_TOOLKIT \
											-DCUDALIB_LIBRARY:FILEPATH=/usr/lib/x86_64-linux-gnu \
...
`#											-DLIBCXXABI_LIBCXX_INCLUDES:PATH="$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/include/c++/v1"` \
`#											-DLIBCXX_CXX_ABI_INCLUDE_PATHS:PATHS="$WORKING_PATH/$RELATIVE_SOURCE_PATH/projects/libcxxabi/include;$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/include/c++/v1"` \
											-DLIBCXX_CXX_ABI_INCLUDE_PATHS:PATHS="$WORKING_PATH/$RELATIVE_SOURCE_PATH/projects/libcxxabi/include" \
...
`#											-DCMAKE_CXX_FLAGS:STRING="-std=c++11 -stdlib=libc++ -I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/lib/clang/$CLANG_MAIN_VERSION.0.0/include -I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/include/c++/v1"` \
											-DCMAKE_CXX_FLAGS:STRING="-stdlib=libc++ -I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/lib/clang/$CLANG_MAIN_VERSION.0.0/include -I$WORKING_PATH/$RELATIVE_SOURCE_PATH/projects/libcxx/include" \
`#											-DCMAKE_C_FLAGS:STRING="-I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/lib/clang/$CLANG_MAIN_VERSION.0.0/include -I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/include/c++/v1"` \
											-DCMAKE_C_FLAGS:STRING="-I$WORKING_PATH/$INSTALL_PATH_PREFIX-$RELATIVE_SOURCE_PATH/lib/clang/$CLANG_MAIN_VERSION.0.0/include -I$WORKING_PATH/$RELATIVE_SOURCE_PATH/projects/libcxx/include" \
