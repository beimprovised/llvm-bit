...
CMAKE=$1
BUILD_PATH_PREFIX=$2
INSTALL_PATH_PREFIX=$3

DOXYGEN_DOT_EXECUTABLE=$4 	#/usr/bin/dot
DOXYGEN_EXECUTABLE=$5 		#/usr/bin/doxygen
			...
			if ! [ "$INSTALL_PATH_PREFIX" = "" ] ; then
				...
				if ! [ "$DOXYGEN_DOT_EXECUTABLE" = "" ] ; then
					...
					if ! [ "$DOXYGEN_EXECUTABLE" = "" ] ; then
						...
						if ! [ -d "$WORKING_PAT/$BUILD_PATH_PREFIX-llvm"]; then
							mkdir "$WORKING_PAT/$BUILD_PATH_PREFIX-llvm"
						fi;
						cd "$WORKING_PAT/$BUILD_PATH_PREFIX-llvm"
					...
					`#	-DLLDB_EXPORT_ALL_SYMBOLS:BOOL=ON` \
						...
						echo "Provide \$DOXYGEN_EXECUTABLE from argument 5, please."
					fi;

				else
					echo "Provide \$DOXYGEN_DOT_EXECUTABLE from argument 4, please."
				fi;

			else
				echo "Provide \$INSTALL_PATH_PREFIX from argument 3, please."
			fi;
		else
			echo "Provide \$BUILD_PATH_PREFIX from argument 2, please."
		fi;
	else
